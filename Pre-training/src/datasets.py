...
class SingleImageDepthStereoTrainingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load camera intrinsics
        if focal_length_baseline0_path is not None:
...
class SingleImageDepthResizedTrainingDataset(torch.utils.data.Dataset):
    ...
    Arg(s):
        ...
        ground_truth_paths : list[str]
            paths to depth maps for left camera
    ...
    def __init__(self,
        ...
        # Since we do not have depth for right image, cannot do swap
        if None in ground_truth_paths:
            self.do_random_swap_left_right = False
        else:
            self.do_random_swap_left_right = random_swap_left_right

        self.miscalibration_pct_focal_length_scale = 0.00
        self.miscalibration_pct_extrinsics = 0.00

        self.miscalibration_pct_extrinsics_shift_y = int(np.random.uniform(
            low=-self.miscalibration_pct_extrinsics * 1024,
            high=self.miscalibration_pct_extrinsics * 1024))

        self.miscalibration_pct_extrinsics_shift_x = int(np.random.uniform(
            low=-self.miscalibration_pct_extrinsics * 2048,
            high=self.miscalibration_pct_extrinsics * 2048))

        self.miscalibration_pct_extrinsics_rotate = np.random.uniform(
            low=-self.miscalibration_pct_extrinsics * 100 * 3,
            high=self.miscalibration_pct_extrinsics * 100 * 3)
    ...
    def __getitem__(self, index):
        ...
        if do_swap:
            ...
            ground_truth0_path = self.ground_truth_paths[index]
        ...
        if ground_truth0_path is not None:
            ground_truth0 = data_utils.load_depth(ground_truth0_path, data_format='HW')

            # Mis-calibration in focal length
            if self.miscalibration_pct_focal_length_scale > 0.0:
                focal_length_scale = np.random.normal(
                    loc=0.0,
                    scale=self.miscalibration_pct_focal_length_scale / 3.0)

                ground_truth0 = ground_truth0 * (1 + focal_length_scale)

            # Mis-calibration in mis-calibrated extrinsics
            if self.miscalibration_pct_extrinsics > 0.0:
                ground_truth0 = data_utils.translate_vertical(
                    ground_truth0,
                    translate_by=self.miscalibration_pct_extrinsics_shift_y,
                    data_format='HWC')

                ground_truth0 = data_utils.translate_horizontal(
                    ground_truth0,
                    translate_by=self.miscalibration_pct_extrinsics_shift_x,
                    data_format='HWC')

                ground_truth0 = data_utils.rotate(
                    ground_truth0,
                    angle=self.miscalibration_pct_extrinsics_rotate,
                    interp_method='nearest',
                    data_format='HWC')

        else:
            ground_truth0 = np.zeros([height, width])

        ground_truth0 = np.expand_dims(ground_truth0, axis=-1)
        images = [
            image0,
            image1,
            image2,
        ...
        images = [
            np.transpose(image, (2, 0, 1)).astype(np.float32) for image in images
        ]

        images, [intrinsics0] = random_crop(
            inputs=images,
            shape=self.resize_shape,
            intrinsics=[intrinsics0],
            crop_type=['horizontal', 'vertical'])

        ground_truth0 = images[-1]
        images = images[:-1]
        images = [
        ...
        outputs = images + [
            intrinsics0.astype(np.float32),
            focal_length_baseline0.astype(np.float32),
            ground_truth0.astype(np.float32)
        ]
...
class OpticalFlowResizedTrainingDataset(torch.utils.data.Dataset):
    '''
    Dataset for fetching:
        (1) camera image at t
    ...
    def __getitem__(self, index):
        ...
        # Load images: t, t-1, t+1
        image0, image1, _ = load_triplet_image(
            path=image0_path,
            normalize=False,
            data_format=self.data_format)
...
class ReconstructionTrainingDataset(torch.utils.data.Dataset):
    '''
    Dataset for fetching:
        (1) RGB image
    ...
    def __getitem__(self, index):
        ...
        # Switch from H x W x C to C x H x W
        image0 = np.transpose(image0, (2, 0, 1)).astype(np.uint8)
...
class Monodepth2TrainingDataset(torch.utils.data.Dataset):
    '''
    Dataset for fetching images
    ...
    def __getitem__(self, index):
        ...
        if self.ground_truth_paths[index] is not None:
            ground_truth0 = data_utils.load_depth(self.ground_truth_paths[index])
            ground_truth0 = np.expand_dims(ground_truth0, axis=-1)
        else:
            ground_truth0 = np.zeros([image_info['height'], image_info['width'], 1])

        if self.load_image_triplets:

            # Crop image, depth and adjust intrinsics
            if self.do_random_crop:
                image0, image1, image2, ground_truth0 = [
                    np.transpose(image, (2, 0, 1))
                    for image in [image0, image1, image2, ground_truth0]
                ]

                [image0, image1, image2, ground_truth0], [intrinsics] = random_crop(
                    inputs=[image0, image1, image2, ground_truth0],
                    shape=self.random_crop_shape,
            ...
            # Crop image, depth and adjust intrinsics
            if self.do_random_crop:
                image0, ground_truth0 = [
                    np.transpose(image, (2, 0, 1))
                    for image in [image0, ground_truth0]
                ]

                [image0, ground_truth0], [intrinsics] = random_crop(
                    inputs=[image0, ground_truth0],
                    shape=self.random_crop_shape,
                    intrinsics=[intrinsics],
                    crop_type=self.random_crop_type)

                image0, ground_truth0 = [
                    np.transpose(image0, (1, 2, 0))
                    for image in [image0, ground_truth0]
                ]

            # Resize inputs
            [image0], [intrinsics] = resize(
                inputs=[image0],
                shape=self.resize_shape,
                focal_length_baseline=None,
                intrinsics=[intrinsics])
        ...
        if self.return_image_info:
            return images + [intrinsics.astype(np.float32)] + [ground_truth0] + [image_info]
        else:
            return images + [intrinsics.astype(np.float32)] + [ground_truth0]
